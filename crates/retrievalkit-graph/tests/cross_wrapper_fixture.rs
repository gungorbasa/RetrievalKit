    ...
    let traversal_result = graph
        .graph_query(
            &GraphQuery::new(Seed::NodeIds(vec![NodeId::record(
                ...
                retrievalkit_core::RecordId::new(&traversal.seed_record_id).unwrap(),
            )]))
            .traverse(Traverse {
    ...
    let all_result = graph
        .graph_query(
            &GraphQuery::new(Seed::Equals {
                ...
                field: FieldPath::new(vec![retrievalkit_core::FieldName::new("title").unwrap()])
                    .unwrap(),
                values: filtered
                    .seed_titles
                    .iter()
...
fn record_ids(matches: &[retrievalkit_graph::GraphMatch]) -> Vec<String> {
    matches
        .iter()
