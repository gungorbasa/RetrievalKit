...
mod tests {
    ...
    use vectorkit_core::{ChunkIdentity, RecordId};
    use vectorkit_graph::{Direction, GraphQuery, NodeId, QueryLimits, Seed, Traverse};
    ...
    fn maps_frozen_schema_and_builds_graph_only_database() {
        ...
        assert_eq!(
            ...
            vectorkit_graph::GraphBuildStats {
                records: 7,
                nodes: 15,
                edges: 26,
                diagnostics: 0,
        ...
        assert_eq!(
            database.corpus().chunk_id_for_identity(&ChunkIdentity::new(
                ...
                vectorkit_core::ChunkKey::new("details").unwrap(),
            )),
            Some(0)
        );
    }
    ...
    fn stable_query_result(
        database: &GraphDatabase,
    ) -> Vec<(NodeId, Vec<vectorkit_graph::GraphPathEdge>)> {
