...
impl TestDirectory {
    fn new() -> Self {
        ...
        let path = std::env::temp_dir().join(format!(
            "vectorkit-phase7-regression-{}-{nonce}",
            std::process::id()
        ));
...
fn record_ids(hits: &[vectorkit_core::SearchHit]) -> Vec<String> {
    hits.iter().map(|hit| hit.document_id.clone()).collect()
}

fn hybrid_ids(hits: &[vectorkit_core::HybridHit]) -> Vec<String> {
    hits.iter().map(|hit| hit.document_id.clone()).collect()
}
