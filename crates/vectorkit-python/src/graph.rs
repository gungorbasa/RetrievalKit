...
use vectorkit_core::{
    ...
    RetrievalDatabase, SearchHit, SearchQuery, VectorKitError as CoreError,
};
use vectorkit_graph::{
...
use crate::{
    ...
    RetrievalCapabilityUnavailableError, StaleGraphSelectionError, VectorKitError,
};
...
impl PyGraphRetrievalDatabaseBuilder {
    ...
    fn new(
        ...
        let vector = vectorkit_core::IndexConfig::new(dimension, parse_metric(metric)?)
...
fn parse_query(value: &str) -> PyResult<GraphQuery> {
    ...
    let seed = match request.seed {
        ...
        QuerySeed::Equals {
            ...
            field: vectorkit_graph::FieldPath::new(
                field
                    .into_iter()
                    .map(|field| vectorkit_core::FieldName::new(field).map_err(core_error))
                    .collect::<PyResult<Vec<_>>>()?,
            )
            .map_err(graph_error)?,
