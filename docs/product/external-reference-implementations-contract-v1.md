...
the fewer-than-50K V1 product boundary, authorize ANN in production RetrievalKit,
...
1. **Exact engine isolation:** RetrievalKit F32 exact and sqlite-vec brute-force
...
3. **Complete application:** RetrievalKit graph selection and scoped retrieval are
   ...
   semantically identical to RetrievalKit, so hybrid rankings are
...
production dependency, alter `retrievalkit-core`, add ANN/HNSW to RetrievalKit, or
...
| --- | --- | --- | --- | --- |
| `retrievalkit_f32_exact` | source commit recorded per run | product exact reference | repository MIT | measures the production exact engine without adding an adapter to production code |
| `retrievalkit_graph_app` | source commit recorded per run | product complete-application reference | repository MIT | measures production graph selection and scoped retrieval in a process isolated from the graph-free Python distribution |
...
RetrievalKit wheels are built from this repository, but no Swift/device harness is
used by Phase 5. Maturin `1.14.1` is the pinned local RetrievalKit wheel builder.
...
stability, but is not required to equal RetrievalKit because its tokenizer, BM25,
...
RetrievalKit bindings, recall failures, build/load errors, schema mismatches,
...
The final acceptance gate requires successful exact RetrievalKit and sqlite-vec
...
- comparing unfiltered ANN with a filtered RetrievalKit query;
- treating FTS5/application fusion as RetrievalKit hybrid parity;
...
- adding ANN/HNSW to production RetrievalKit; or
