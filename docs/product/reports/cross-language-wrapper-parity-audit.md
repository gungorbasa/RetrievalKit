...
| Graph excluded from base package | Off-by-default Cargo feature | Separate `VectorKitGraph` package and aggregate XCFramework | Separate `vectorkit-graph` distribution | Aligned |
...
- Preserved graph-free base packaging and verified that the base wheel contains
  neither graph Python files nor a `vectorkit-graph` native dependency.
