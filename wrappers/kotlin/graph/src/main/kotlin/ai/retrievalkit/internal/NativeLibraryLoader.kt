        ...
        try {
            ...
            val stream = NativeLibraryLoader::class.java.getResourceAsStream(resource)
                ?: throw NativeLibraryException(
                    "native library resource $resource is missing; build the graph native aggregate " +
                        "or supply -Dretrievalkit.native.path",
                )
            val temporary = Files.createTempFile("retrievalkit-graph-", mapped.substringAfterLast('.'))
