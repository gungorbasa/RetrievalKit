...
private final class NativeGraphExecution: @unchecked Sendable {
  ...
  func close() {
    ...
    result = nil
    condition.unlock()
    if let ownedScope { retrievalkit_graph_scope_free(OpaquePointer(bitPattern: ownedScope)) }
    if let ownedResult { retrievalkit_graph_result_free(OpaquePointer(bitPattern: ownedResult)) }
  }
  func withScope<T>(_ body: (OpaquePointer) throws -> T) throws -> T {
    ...
    guard !closed, let scope, let pointer = OpaquePointer(bitPattern: scope) else {
      condition.unlock()
      throw RetrievalKitGraphError.graphUnavailable("graph query result is closed")
    }
    activeUses += 1
    condition.unlock()
    defer {
...
public final class GraphCancellationToken: @unchecked Sendable {
  private let condition = NSCondition()
  private var handle: UInt? = UInt(bitPattern: retrievalkit_graph_cancellation_new())
  private var activeUses = 0
  private var closed = false
  public init() {}
  deinit { close() }
  public func cancel() {
    ...
    if let handle { retrievalkit_graph_cancellation_cancel(OpaquePointer(bitPattern: handle)) }
  }
  public func close() {
    condition.lock()
    closed = true
    ...
    handle = nil
    condition.unlock()
    if let owned { retrievalkit_graph_cancellation_free(OpaquePointer(bitPattern: owned)) }
  }
  fileprivate func withPointer<T>(_ body: (OpaquePointer) throws -> T) throws -> T {
    ...
    guard !closed, let handle, let pointer = OpaquePointer(bitPattern: handle) else {
      condition.unlock()
      throw RetrievalKitGraphError.graphUnavailable("graph cancellation token is closed")
    }
    activeUses += 1
    condition.unlock()
    defer {
...
private final class NativeGraphFilter {
  ...
  deinit { retrievalkit_filter_free(pointer) }
}
...
extension GraphFilter {
  ...
  fileprivate func makeFFIPointer() throws -> OpaquePointer {
    ...
    case .exists(let field):
      try makeLeaf { status, arena in retrievalkit_filter_exists(arena.copy(field), status) }
      ...
      try makeLeaf { status, arena in
        ...
        return withOptionalGraphPointer(to: &lowerValue) { lowerPointer in
          withOptionalGraphPointer(to: &upperValue) { upperPointer in
            retrievalkit_filter_range(arena.copy(field), lowerPointer, upperPointer, status)
          }
        }
      }
    case .inValues(let field, let values):
      try makeLeaf { status, arena in
        ...
        return nativeValues.withUnsafeBufferPointer { buffer in
          retrievalkit_filter_in_values(arena.copy(field), buffer.baseAddress, buffer.count, status)
        }
      }
    case .all(let filters): try makeComposite(filters, builder: retrievalkit_filter_all)
    case .any(let filters): try makeComposite(filters, builder: retrievalkit_filter_any)
    }
  }
...
private final class NativeGraphIndexOwner: @unchecked Sendable {
  ...
  func requireHandle() throws -> UInt {
    lock.lock()
    defer { lock.unlock() }
    guard let handle else { throw RetrievalKitGraphError.graphUnavailable("graph index is closed") }
    return handle
  }
  func close() {
    lock.lock()
    ...
    handle = nil
    lock.unlock()
    if let owned { retrievalkit_graph_index_free(OpaquePointer(bitPattern: owned)) }
  }
}
...
public actor GraphIndexBuilder {
    ...
    handle = UInt(
      bitPattern: try Native.pointer { status in
        corpusID.withCString {
          retrievalkit_graph_builder_new(dimension, metric.ffi, encoding.ffi, $0, status)
        }
      })
  }
  deinit { if let handle { retrievalkit_graph_builder_free(OpaquePointer(bitPattern: handle)) } }
  public func close() {
    if let handle {
      retrievalkit_graph_builder_free(OpaquePointer(bitPattern: handle))
      self.handle = nil
    }
    closed = true
  }
  public func upsert(_ batch: GraphRecordBatch) throws {
    if closed { throw RetrievalKitGraphError.graphUnavailable("graph builder is closed") }
    guard let handle else { throw RetrievalKitGraphError.consumedBuilder }
    let data = try JSONEncoder().encode(batch)
    let json = String(decoding: data, as: UTF8.self)
    try Native.bool { status in
      json.withCString {
        retrievalkit_graph_builder_upsert_record_json(OpaquePointer(bitPattern: handle), $0, status)
      }
    }
  }
  public func build(schema: GraphSchema) throws -> GraphIndex {
    ...
    let graph = try Native.pointer { status in
      json.withCString {
        retrievalkit_graph_builder_build_json(OpaquePointer(bitPattern: owned), $0, status)
      }
    }
...
public actor GraphIndex {
      ...
      try await Task.detached(priority: Task.currentPriority) {
        ...
        try Native.bool { status in
          directory.path.withCString {
            retrievalkit_graph_index_save(OpaquePointer(bitPattern: handle), $0, status)
          }
        }
      }.value
    }
  ...
  public static func load(from directory: URL) throws -> GraphIndex {
    GraphIndex(
      handle: UInt(
        bitPattern: try Native.pointer { status in
          directory.path.withCString { retrievalkit_graph_index_load($0, status) }
        }))
  }
  public static func validate(at directory: URL) throws {
    try Native.bool { status in
      directory.path.withCString { retrievalkit_graph_index_validate($0, status) }
    }
  }
  ...
  private nonisolated static func execute(
    ...
    let result: OpaquePointer? =
      if let cancellation {
        try cancellation.withPointer { pointer in
          retrievalkit_graph_query(OpaquePointer(bitPattern: handle), query, pointer, &status)
        }
      } else {
        retrievalkit_graph_query(OpaquePointer(bitPattern: handle), query, nil, &status)
      }
    ...
    guard
      let scope = retrievalkit_graph_result_project(OpaquePointer(bitPattern: handle), result, &status)
    else {
      defer {
        retrievalkit_status_clear(&status)
    ...
    let projection = GraphProjectionTrace(
      sourceNodes: retrievalkit_graph_scope_source_nodes(scope),
      resolvedChunks: retrievalkit_graph_scope_resolved_chunks(scope))
    ...
    for index in 0..<retrievalkit_graph_result_count(result) {
      ...
      retrievalkit_graph_match_clear(&value)
      var path: [GraphPathEdge] = []
      for edgeIndex in 0..<pathLength {
        var edge = emptyPathEdge()
        try Native.bool { status in
          retrievalkit_graph_result_path_edge(result, index, edgeIndex, &edge, status)
        }
        let sourceField =
        ...
        retrievalkit_graph_path_edge_clear(&edge)
      }
      matches.append(GraphMatch(nodeID: nodeID, depth: depth, path: path))
    }
    let trace = retrievalkit_graph_result_trace(result)
...
private func requireNonnegative(_ value: Int, name: String) throws {
  guard value >= 0 else {
    throw RetrievalKitGraphError.invalidIdentity("\(name) must not be negative")
  }
}
...
private final class NativeCapabilityDatabase: @unchecked Sendable {
  ...
  func requireHandle() throws -> UInt {
    lock.lock()
    defer { lock.unlock() }
    guard let handle else { throw RetrievalKitGraphError.graphUnavailable("database is closed") }
    return handle
  }
  func close() {
    lock.lock()
    ...
    switch kind {
    case .graph: retrievalkit_graph_database_free(OpaquePointer(bitPattern: owned))
    case .graphRetrieval: retrievalkit_graph_retrieval_database_free(OpaquePointer(bitPattern: owned))
    }
  }
}
private final class NativeSelection: @unchecked Sendable {
  ...
  deinit {
    ...
    handle = nil
    lock.unlock()
    if let owned { retrievalkit_graph_result_free(OpaquePointer(bitPattern: owned)) }
  }
  func requireHandle() throws -> OpaquePointer {
    lock.lock()
    defer { lock.unlock() }
    guard let handle, let pointer = OpaquePointer(bitPattern: handle) else {
      throw RetrievalKitGraphError.graphUnavailable("graph selection is closed")
    }
    return pointer
  }
}
...
public actor GraphQueries {
  ...
  private nonisolated static func execute(
    ...
    let call: (OpaquePointer?) throws -> OpaquePointer? = { cancellationPointer in
      switch owner.kind {
      case .graph:
        retrievalkit_graph_database_query(
          OpaquePointer(bitPattern: handle), query, cancellationPointer, &status)
      case .graphRetrieval:
        retrievalkit_graph_retrieval_database_query(
          OpaquePointer(bitPattern: handle), query, cancellationPointer, &status)
      }
    }
    let result =
...
private func materializeSelection(_ result: OpaquePointer) throws -> GraphSelection {
  ...
  for index in 0..<retrievalkit_graph_result_count(result) {
    ...
    for edgeIndex in 0..<pathLength {
      var edge = emptyPathEdge()
      try Native.bool { retrievalkit_graph_result_path_edge(result, index, edgeIndex, &edge, $0) }
      let sourceField =
        edge.source_field_segments.count == 0
        ? nil
        : GraphFieldPath(
...
private func materializeCandidateProjection(
  ...
  for index in 0..<output.count {
    ...
    guard
      ...
      throw RetrievalKitGraphError.internalError("native projection returned an invalid identity")
    }
...
public actor GraphDatabase {
      ...
      handle = UInt(
        bitPattern: try Native.pointer { status in
          corpusID.rawValue.withCString { corpus in
            json.withCString { retrievalkit_graph_database_builder_new(corpus, $0, status) }
          }
        })
    }
    deinit {
    ...
    public func build() throws -> GraphDatabase {
      ...
      return GraphDatabase(
        pointer: try Native.pointer {
          retrievalkit_graph_database_builder_build(OpaquePointer(bitPattern: owned), $0)
        })
    }
  }
  private let owner: NativeCapabilityDatabase
  ...
  public func save(to directory: URL) async throws {
    ...
    try await Task.detached {
      ...
      guard
        directory.path.withCString({
          retrievalkit_graph_database_save(OpaquePointer(bitPattern: handle), $0, &status)
        })
  ...
  public static func load(from directory: URL) throws -> GraphDatabase {
    GraphDatabase(
      pointer: try Native.pointer { status in
        directory.path.withCString { retrievalkit_graph_database_load($0, status) }
      })
  }
  public static func validate(at directory: URL) throws {
    try Native.bool { status in
      directory.path.withCString { retrievalkit_graph_database_validate($0, status) }
    }
  }
}
...
public actor GraphRetrievalDatabase {
  ...
  public func save(to directory: URL) async throws {
    ...
    try await Task.detached {
      ...
      guard
        directory.path.withCString({
          retrievalkit_graph_retrieval_database_save(OpaquePointer(bitPattern: handle), $0, &status)
        })
  ...
  public static func load(from directory: URL) throws -> GraphRetrievalDatabase {
    GraphRetrievalDatabase(
      pointer: try Native.pointer { status in
        directory.path.withCString { retrievalkit_graph_retrieval_database_load($0, status) }
      })
  }
  public static func validate(at directory: URL) throws {
    try Native.bool { status in
      directory.path.withCString { retrievalkit_graph_retrieval_database_validate($0, status) }
    }
  }
}
