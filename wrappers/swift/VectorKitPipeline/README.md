...
`VectorKitPipeline` is the optional high-level Swift package that composes:

- `VectorKitIngest` for shared Rust text chunking.
- `EmbeddingKit` for provider-neutral batch and query embeddings.
- `VectorKit` for indexing, persistence, filtering, and hybrid retrieval.
...
```bash
swift run --package-path wrappers/swift/VectorKitPipeline vectorkit-pipeline-example
```

```swift
import EmbeddingKit
import VectorKit
import VectorKitPipeline
...
- `vectorkit.chunk.index`
- `vectorkit.chunk.start_byte`
- `vectorkit.chunk.end_byte`

## Custom Chunking

Applications can replace the built-in Rust chunker by implementing
`DocumentChunker`, which is owned and validated by `VectorKitPipeline`:
